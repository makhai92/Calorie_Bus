...
public class InqueryDao {
	...
	public List selectMyInqueryList(int memberNo, int start, int end) {
		String query = "select * from (select rownum rnum, i.* from ((select inquery_no, inquery_title, reply_no as inquery_content, inquery_date, member_no from inquery left join reply using (inquery_no) where member_no = ?) order by 1)i) where rnum between ? and ?";
